...
export default function HiringMode() {
  const { setHiringMode } = useIDE();

  return (
    <div className="min-h-screen bg-editor-bg text-editor-fg font-sans overflow-y-auto overflow-x-hidden selection:bg-selection selection:text-editor-fg relative">
      ...
      <div className="fixed top-6 right-6 z-50">
        <button
          onClick={() => setHiringMode(false)}
          className="px-4 py-2 bg-sidebar-bg/80 backdrop-blur-md border border-border/50 rounded-full text-[13px] hover:bg-hover transition-all flex items-center gap-2 text-activitybar-fg hover:text-editor-fg hover:scale-105 shadow-lg hover:shadow-accent/10 hover:border-border"
        >
          <Code size={16} />
          <span className="font-medium">Return to IDE</span>
        </button>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-24 relative z-10">
        ...
        >
          ...
          <motion.header variants={itemVariants} className="text-center max-w-2xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-6 py-2.5 bg-accent text-button-fg border border-accent rounded-full text-[14px] font-medium hover:brightness-110 transition-all shadow-lg shadow-accent/20 hover:shadow-accent/40"
              >
          ...
          <motion.section variants={itemVariants}>
            ...
            <div className="space-y-6">
              {experience.map((exp, i) => (
                <div 
                  key={i}
                  className="p-8 bg-sidebar-bg/40 backdrop-blur-sm border border-border/50 rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/10 hover:border-accent/30 group"
                >
                  <div className="flex flex-col md:flex-row md:justify-between md:items-baseline mb-4 gap-2 md:gap-0">
                    <div>
                      ...
                      <div className="text-activitybar-active font-medium mt-1">{exp.company}</div>
                    </div>
          ...
          <motion.footer variants={itemVariants} className="pt-10 pb-8 text-center text-[13px] text-activitybar-fg border-t border-border/30">
